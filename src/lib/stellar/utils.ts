 ...
 * (see the nft_oz payment design's Part B/C) rather than the wallet
 * submitting its own transaction and paying its own network fee. Never
 * called for a custodial `walletType` — the server handles those entirely
 * itself and never hands back an XDR to sign client-side.
 */
...
/**
 * Extracts the transaction hash from whatever shape `clientsign` returns.
 * Horizon responses and custodial submit use `.hash`; Albedo uses `.tx_hash`.
 */
