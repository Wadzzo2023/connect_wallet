...
export async function clientsign(props: {
  ...
  switch (props.walletType) {
    ...
    case WalletType.walletConnect:
      return await walletConnectSignTransactionSubmitterWrapper(props.presignedxdr)
...
export function concatAssetWithIssuer(asset_code: string, asset_issuer: string): string {
  return `${asset_code}-${asset_issuer}`
}

export function getAssetIssuerFromConcat(concatenatedStr: string): [string, string] {
...
export async function checkStellarAccountActivity(publicKey: string): Promise<boolean> {
  try {
    const response = await axios.get(`${STELLAR_URL}/accounts/${publicKey}`)
    const accountData = response.data

    // Check if the account exists and has a balance
    if (accountData && accountData.id && accountData.balances && accountData.balances.length > 0) {
